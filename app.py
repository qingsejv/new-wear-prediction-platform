...
MODEL_CANDIDATES = [
    BASE_DIR / "models" / "friction_wear_model.joblib",
    BASE_DIR / "friction_wear_model.joblib",
]

# ---------- Matplotlib font ----------
# Use English labels in matplotlib charts, so no extra Chinese font file is required on Streamlit Cloud.
...
PLOT_FEATURE_DISPLAY_NAMES = {
    ...
    "磨损载荷 N": "Wear load (N)",
    "磨损载荷 kN": "Wear load (kN)",
    "镀层实际厚度 μm": "Coating thickness (um)",
    "粗糙度Rz μm": "Roughness Rz (um)",
}
...
def display_feature_name(col):
    ...
    return FEATURE_DISPLAY_NAMES.get(
        col,
        col.replace("粗糙度Rz", "粗糙度 Rz")
    )


def plot_feature_name(col):
    """Matplotlib 图表统一使用英文，避免云端中文字体显示为方框。"""
    return PLOT_FEATURE_DISPLAY_NAMES.get(col, col)
...
def load_model_package():
    ...
    st.error(
        "未找到模型文件。请把 friction_wear_model.joblib 放在项目根目录，"
        "或放在 models/friction_wear_model.joblib。"
    )
    st.stop()
...
with mid:
    ...
    if pred_wear is not None:
        ...
        <div class="metric-card highlight wear">
            <div class="metric-label">预测磨损质量</div>
            <div class="metric-value">{pred_wear:.4f}</div>
            <div class="metric-sub">单位：mg，由 wear_model 预测输出</div>
        </div>
        """, unsafe_allow_html=True)
    else:
        st.warning("未检测到 wear_model，无法输出磨损质量。")
...
with right:
    ...
    <div class="summary-row"><span>预测目标</span><b>磨损质量 / 平均摩擦系数</b></div>
    ...
    if pred_friction is not None:
        ...
        try:
            ...
            ax.plot(
                ...
                preds,
                linewidth=2.5,
                label="Predicted friction coefficient"
            )
            ax.scatter(
            ...
            x_label = plot_feature_name(selected_feature)

            ax.set_xlabel(x_label)
            ax.set_ylabel("Average friction coefficient")
            ax.set_title("One-factor response trend")
            ax.grid(alpha=0.25)
...
with chart_col2:
    ...
    if pred_wear is not None:
        st.markdown(f'<div class="summary-row"><span>磨损质量 mg</span><b>{pred_wear:.4f}</b></div>', unsafe_allow_html=True)
    if pred_friction is not None:
